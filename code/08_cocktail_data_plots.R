...
choice_sets_to_show = 1:7

for(i in 0:(length(choice_sets_to_show) + 1)){
  
  if(i == length(choice_sets_to_show) + 1){
    choice_sets_i = 1:length(unique(cocktail_pv_d_opt_df$choice_set))
  } else{
    if(i == 0){
      choice_sets_i = NULL
    } else{
  ...
  J = dim(cocktail_pv_d_opt$X)[2]
  
  plot_d = cocktail_pv_d_opt_df %>% 
    filter(choice_set %in% choice_sets_i) %>% 
    mutate(choice_set = as.character(as.integer(gl(length(choice_sets_i), J)))) %>% 
    ggtern::ggtern(ggtern::aes(x1, x2, x3, color = z1, label = choice_set)) +
  ...
  ### One by one
  ggtern::ggsave(
    filename = paste0(plots_folder, "simplex_plot_cocktail_pv_D_optimal_", max_it_cocktail, "iter_num", i, ".jpg"),
    plot = plot_d,
  ...
  ggtern::ggsave(
    filename = paste0(plots_folder, "simplex_plot_cocktail_pv_I_optimal_", max_it_cocktail, "iter_num", i, ".jpg"),
    plot = plot_i,
    width = width_cocktail_simplex,
    height = height_cocktail_simplex,
    units = "cm"
  ...
  plot_both = ggtern::grid.arrange(
    plot_d + theme(legend.position = "none", legend.box = "none"), 
    plot_i + theme(legend.position = "none", legend.box = "none"), 
    ncol = 2
  )
  
  ggtern::ggsave(
    filename = paste0(plots_folder, "simplex_plot_cocktail_pv_both_", max_it_cocktail, "iter_num", i, ".jpg"),
    plot = plot_both,
