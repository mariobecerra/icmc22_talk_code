...
max_it_cocktail = 15
cocktail_pv_d_opt_filename = paste0(designs_folder, "cocktail_pv_d_optimal_small_", max_it_cocktail, "iter.rds")
cocktail_pv_i_opt_filename = paste0(designs_folder, "cocktail_pv_i_optimal_small_", max_it_cocktail, "iter.rds")
...
mnl_plot_result(cocktail_pv_d_opt)
mnl_plot_result(cocktail_pv_i_opt)
...
(cocktail_pv_d_opt_df %>%
  ...
  ggtern::ggsave(
    plot = ., 
    filename = paste0(plots_folder, "simplex_plot_cocktail_pv_D_optimal_small_", max_it_cocktail, "iter.jpg"),
    width = simplex_width,
...
(cocktail_pv_i_opt_df %>%
  ...
  ggtern::ggsave(
    plot = .,
    filename = paste0(plots_folder, "simplex_plot_cocktail_pv_I_optimal_small_", max_it_cocktail, "iter.jpg"),
    width = simplex_width,
    height = simplex_height,
    units = "cm"
  )
...
(cocktail_pv_i_opt_df %>% 
    ...
    ggplot() +
    geom_dotplot(aes(z1, fill = design, color = design), method = 'histodot', binwidth = 0.16) +
    facet_wrap(~design) +
    theme_bw() +
  ...
  ggsave(
    plot = .,
    filename = paste0(plots_folder, "process_variables_dotplot_small_", max_it_cocktail, "iter.jpg"),
    width = 12,
    height = 8,
    units = "cm"
  )
...
ggplot2::ggsave(
  filename = paste0(plots_folder, "res_cocktail_pv_small_fds_db_vs_ib_plot.png"),
...
(
  ...
  ggplot2::ggsave(
    filename = paste0(plots_folder, "res_cocktail_pv_small_distances_within_choice_set.png"),
    plot = .,
    width = 8,
    height = 9,
    units = "cm"
